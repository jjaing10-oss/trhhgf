...
function initDashboard(){
  try{
    ...
    const avgKpi = K.length ? (K.reduce((s,x)=>s+(Number(x.ts)||0),0)/K.length).toFixed(1) : '-';

    const el = (id)=>document.getElementById(id);

    if(el('dashRevenue')) el('dashRevenue').textContent = (revenue/1e8).toFixed(1)+'억';
    if(el('dashRevenueSub')) el('dashRevenueSub').textContent = '매출총이익 '+(gross/1e8).toFixed(1)+'억';
    
    if(el('dashOp')) el('dashOp').textContent = (op/1e8).toFixed(1)+'억';
    if(el('dashOpSub')) el('dashOpSub').textContent = '완료 과제 '+done+'개';

    if(el('dashMargin')) el('dashMargin').textContent = margin+'%';
    if(el('dashMarginSub')) el('dashMarginSub').textContent = '진행중 '+T.filter(t=>t.st==='진행중').length+'개';

    if(el('dashKpi')) el('dashKpi').textContent = avgKpi+'점';
