...
function initDashboard(){
  ...
  const revenue=P.revenue?.total||0;
  const op=P.op?.total||0;
  const margin=revenue?((op/revenue)*100).toFixed(1):'0.0';
  const avgKpi=K.length?(K.reduce((s,k)=>s+(Number(k.ts)||0),0)/K.length).toFixed(1):'-';
  ...
  const mgmt=C.jan26?.mgmt_fee?.total||0;
  const mgmtBase=C.jan26?.mgmt_fee_25jan||0;
  const mgmtYoY=mgmtBase?(((mgmt-mgmtBase)/mgmtBase)*100).toFixed(1):null;

  const set=(id,html)=>{ const el=document.getElementById(id); if(el) el.innerHTML=html; };
  set('dashMonth', document.getElementById('hdrMonthBadge')?.textContent || '2026.01');
  set('dashRevenue', `${fB(revenue)}억`);
  set('dashRevenueSub', `총이익 ${fB(P.gross?.total||0)}억`);
  set('dashOp', `${fB(op)}억`);
  set('dashOpSub', `완료 과제 ${done}개 / 진행중 ${prog}개`);
  set('dashMargin', `${margin}%`);
  set('dashMarginSub', `핵심 미완료 ${corePending.length}개`);
  set('dashKpi', `${avgKpi}점`);
  set('dashKpiSub', `최고 ${best.hq} · 최저 ${worst.hq}`);

  set('dashBriefing', `
    <div class="dash-brief-item">
      <div class="dash-brief-badge">손익</div>
      <div class="dash-brief-text"><b>${fB(op)}억</b> 영업이익, 이익률 <b>${margin}%</b> 수준입니다. 점프업·도소매 조정 후 기준으로 보면 현재 수익성은 방어 중입니다.</div>
    </div>
    <div class="dash-brief-item">
      <div class="dash-brief-badge">KPI</div>
      <div class="dash-brief-text">평균 <b>${avgKpi}점</b>. 최고는 <b>${best.hq}</b>, 최저는 <b>${worst.hq}</b>라서 본부 간 편차 관리가 핵심입니다.</div>
    </div>
    <div class="dash-brief-item">
      <div class="dash-brief-badge">과제</div>
      <div class="dash-brief-text">전체 <b>${T.length}개</b> 과제 중 완료 <b>${done}개</b>, 핵심 미완료 <b>${corePending.length}개</b>입니다. 그냥 바쁘기만 한 상태로 가면 안 되고 우선순위 재정렬이 필요합니다.</div>
    </div>
    <div class="dash-brief-item">
      <div class="dash-brief-badge">수수료</div>
      <div class="dash-brief-text">1월 관리수수료는 <b>${mgmt?mgmt.toFixed(1):'-'}억</b>${mgmtYoY!==null?`으로 전년동월 대비 <b>${mgmtYoY}%</b>`:''} 흐름입니다.</div>
    </div>`);

  set('dashFocusGrid', `
    <div class="dash-focus-card">
      <div class="dash-focus-top"><span>🏆 KPI 최고</span><b>${best.hq}</b></div>
    ...
    <div class="dash-focus-card warn">
      <div class="dash-focus-top"><span>⚠️ KPI 최저</span><b>${worst.hq}</b></div>
      <div class="dash-focus-main">${Number(worst.ts||0).toFixed(1)}점</div>
      <div class="dash-focus-sub">편차 원인분석 우선 대상</div>
    </div>
    <div class="dash-focus-card">
      <div class="dash-focus-top"><span>📌 핵심 미완료</span><b>${corePending.length}개</b></div>
      <div class="dash-focus-main">${corePending[0]?corePending[0].nm:'이슈 없음'}</div>
      <div class="dash-focus-sub">핵심과제 추진도 집중 점검</div>
    </div>
    <div class="dash-focus-card good">
      <div class="dash-focus-top"><span>✅ 완료 과제</span><b>${done}개</b></div>
      <div class="dash-focus-main">${T.length?((done/T.length)*100).toFixed(1):'0.0'}%</div>
      <div class="dash-focus-sub">전체 과제 완료율</div>
    </div>`);

  const issues = [];
  issues.push({tag:'HIGH', title:'핵심과제 미완료 관리', desc:`핵심과제 ${corePending.length}개가 아직 진행중입니다. 추진도 낮은 과제부터 팀장 회의 안건으로 올리는 편이 낫습니다.`});
  issues.push({tag:'WATCH', title:'KPI 편차 확대', desc:`${best.hq}와 ${worst.hq} 간 점수 차가 ${(Number(best.ts||0)-Number(worst.ts||0)).toFixed(1)}점입니다. 본부별 액션플랜 정리가 필요합니다.`});
  issues.push({tag:'CHECK', title:'수익성 방어', desc:`영업이익률 ${margin}% 수준으로 비용구조와 채널믹스 점검이 계속 필요합니다.`});
  if(mgmtYoY!==null) issues.push({tag:mgmtYoY.startsWith('-')?'WARN':'INFO', title:'관리수수료 흐름', desc:`1월 관리수수료 전년동월 대비 ${mgmtYoY}% ${mgmtYoY.startsWith('-')?'하락':'증가'}했습니다.`});
  set('dashIssues', issues.map(i=>`<div class="dash-issue-item"><span class="dash-issue-tag ${i.tag.toLowerCase()}">${i.tag}</span><div><div class="dash-issue-title">${i.title}</div><div class="dash-issue-desc">${i.desc}</div></div></div>`).join(''));
}
