...
function initBriefing(){
  ...
  const K=E.K||[], P=E.P||{}, T=E.T||[];
  const s=E.sub||{};
  const avgKpi = K.length ? (K.reduce((a,k)=>a+(Number(k.ts)||0),0)/K.length).toFixed(1) : '-';
  const doneRate = T.length ? ((E.done/T.length)*100).toFixed(0) : '0';
  const opm = pctStr(ratio(P?.op?.total,P?.revenue?.total));
  const feeYoy = E.mgmtYoY || 'N/A';
  const top = K[0] || null;
  const bottom = K.length ? K[K.length-1] : null;
  const rankColors = ['#f59e0b','#94a3b8','#d97706','#3b82f6','#8b5cf6'];

  const heat = (v,arr)=>{
    const vals=(arr||[]).map(x=>Number(x)||0);
    if(!vals.length) return '#e2e8f0';
    const mx=Math.max(...vals), mn=Math.min(...vals);
    if(v===mx) return '#dcfce7';
    if(v===mn) return '#fee2e2';
    return '#f8fafc';
  };

  const aiLines = [
    `매출 ${fB(nvl(P?.revenue?.total))}억, 영업이익 ${fB(nvl(P?.op?.total))}억으로 영업이익률 ${opm} 수준입니다.`,
    top&&bottom ? `KPI는 ${top.hq}가 1위, ${bottom.hq}가 최하위이며 본부간 격차는 ${E.kpiGap.toFixed(1)}p입니다.` : `KPI 데이터 점검이 필요합니다.`,
    `과제 완료율 ${doneRate}%이며, 미착수 ${E.zeroProgress.length}건 · 핵심잔여 ${E.coreOpen.length}건입니다.`,
    `관리수수료 YoY ${feeYoy}, 관리비중 ${pctStr(E.mgmtMix)} 수준입니다.`
  ];

  document.getElementById('briefingSnapshot').innerHTML=`
  <div class="exec-brief-wrap">
    <div class="exec-brief-head">
      <div>
        <div class="exec-brief-title">KT M&S Executive Briefing</div>
        <div class="exec-brief-sub">${D.period} 기준</div>
      </div>
      <div class="exec-brief-badges">
        <span class="exec-badge high">HIGH ${E.actions.filter(a=>a.priority==='HIGH').length}</span>
        <span class="exec-badge med">MED ${E.actions.filter(a=>a.priority==='MED').length}</span>
      </div>
    </div>

    <div class="exec-kpi-grid">
      <div class="exec-kpi-card tone-blue">
        <div class="exec-kpi-label">매출</div>
        <div class="exec-kpi-value">${fB(nvl(P?.revenue?.total))}억</div>
        <div class="exec-kpi-meta">전사 합계</div>
      </div>
      <div class="exec-kpi-card tone-green">
        <div class="exec-kpi-label">영업이익</div>
        <div class="exec-kpi-value">${fB(nvl(P?.op?.total))}억</div>
        <div class="exec-kpi-meta">영업이익률 ${opm}</div>
      </div>
      <div class="exec-kpi-card tone-purple">
        <div class="exec-kpi-label">평균 KPI</div>
        <div class="exec-kpi-value">${avgKpi}</div>
        <div class="exec-kpi-meta">${top&&bottom?`${top.hq} 1위 · ${bottom.hq} 최하위`:'본부별 집계'}</div>
      </div>
      <div class="exec-kpi-card tone-orange">
        <div class="exec-kpi-label">과제 완료율</div>
        <div class="exec-kpi-value">${doneRate}%</div>
        <div class="exec-kpi-meta">미착수 ${E.zeroProgress.length}건</div>
      </div>
    </div>

    <div class="exec-panel-grid">
      <div class="exec-panel">
        <div class="exec-panel-title">AI Brief</div>
        <div class="exec-brief-lines">
          ${aiLines.slice(0,3).map(line=>`<div class="exec-line">• ${line}</div>`).join('')}
        </div>
      </div>

      <div class="exec-panel">
        <div class="exec-panel-title">Shortcut</div>
        <div class="exec-shortcuts">
          <button class="exec-shortcut-btn" onclick="switchTab('profit')">BM 손익</button>
          <button class="exec-shortcut-btn" onclick="switchTab('kpi')">KPI 분석</button>
          <button class="exec-shortcut-btn" onclick="switchTab('subscriber')">가입자</button>
          <button class="exec-shortcut-btn" onclick="switchTab('commission')">수수료</button>
        </div>
      </div>
    </div>

    <div class="exec-panel" style="margin-top:10px">
      <div class="exec-panel-title">KPI Heatmap</div>
      <div class="exec-heatmap-wrap">
        <table class="exec-heatmap">
          <thead>
            <tr><th>본부</th><th>종합</th><th>소매</th><th>도매</th><th>소상공</th></tr>
          </thead>
          <tbody>
            ${K.map(k=>`
              <tr>
                <td>${k.hq.replace('본부','')}</td>
                <td style="font-weight:800;color:${rankColors[(k.rk||1)-1]||'#334155'}">${(Number(k.ts)||0).toFixed(1)}</td>
                <td><span class="exec-cell" style="background:${heat(k.rt.t,K.map(x=>x.rt.t))}">${(Number(k.rt.t)||0).toFixed(1)}</span></td>
                <td><span class="exec-cell" style="background:${heat(k.wh.t,K.map(x=>x.wh.t))}">${(Number(k.wh.t)||0).toFixed(1)}</span></td>
                <td><span class="exec-cell" style="background:${heat(k.sm.t,K.map(x=>x.sm.t))}">${(Number(k.sm.t)||0).toFixed(1)}</span></td>
              </tr>
            `).join('')}
          </tbody>
        </table>
