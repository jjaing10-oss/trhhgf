...
function initDashboard(){
  if(typeof D==='undefined' || !D.profit || !D.kpi || !D.tasks) return;
  const P=D.profit, K=D.kpi, T=D.tasks, C=D.commission||{};
  const revenue=Number(P.revenue?.total||0);
  const gross=Number(P.gross?.total||0);
  const op=Number(P.op?.total||0);
  const margin=revenue ? (op/revenue*100) : 0;
  const avgKpi=K.length ? (K.reduce((s,k)=>s+(Number(k.ts)||0),0)/K.length) : 0;
  const done=T.filter(t=>t.st==='완료').length;
  const prog=T.filter(t=>t.st==='진행중').length;
  const planned=T.filter(t=>t.st==='계획').length;
  const corePending=T.filter(t=>t.co==='●' && t.st!=='완료');
  const best=K.reduce((a,b)=>(Number(a.ts)||0)>=(Number(b.ts)||0)?a:b, K[0]||{hq:'-',ts:0});
  const worst=K.reduce((a,b)=>(Number(a.ts)||0)<=(Number(b.ts)||0)?a:b, K[0]||{hq:'-',ts:0});
  const gap=(Number(best.ts)||0)-(Number(worst.ts)||0);
  const mgmt=Number(C.jan26?.mgmt_fee?.total||0);
  const mgmtBase=Number(C.jan26?.mgmt_fee_25jan||0);
  const mgmtYoY=mgmtBase ? ((mgmt-mgmtBase)/mgmtBase*100) : null;
  const taskRate=T.length ? (done/T.length*100) : 0;
  const topTask=corePending.sort((a,b)=>(a.pp||0)-(b.pp||0))[0] || T.find(t=>t.st==='진행중') || T[0] || null;
  const bestName=best?.hq || '-';
  const worstName=worst?.hq || '-';

  const setHTML=(id,html)=>{ const el=document.getElementById(id); if(el) el.innerHTML=html; };
  const setText=(id,text)=>{ const el=document.getElementById(id); if(el) el.textContent=text; };
  const fmtPct=(n)=>`${n.toFixed(1)}%`;

  setText('dashMonth', document.getElementById('hdrMonthBadge')?.textContent?.trim() || '2026.01');
  setHTML('dashMeta', [
    `<span class="dash-meta-chip">총 과제 ${T.length}개</span>`,
    `<span class="dash-meta-chip ok">완료 ${done}개</span>`,
    `<span class="dash-meta-chip warn">핵심 미완료 ${corePending.length}개</span>`,
    `<span class="dash-meta-chip info">KPI 편차 ${gap.toFixed(1)}pt</span>`
  ].join(''));
  setHTML('dashLiveFocus', `${corePending.length ? '핵심과제 관리' : '수익성 모니터링'} · ${worstName}`);

  setText('dashRevenue', `${fB(revenue)}억`);
  setText('dashRevenueSub', `매출총이익 ${fB(gross)}억 · 총이익률 ${fmtPct(revenue?gross/revenue*100:0)}`);
  setText('dashRevenueFoot', `채널 확대보다 질 관리가 더 중요합니다.`);

  setText('dashOp', `${fB(op)}억`);
  setText('dashOpSub', `공헌이익 ${fB(Number(P.contribution?.total||op))}억 · 완료 과제 ${done}개`);
  setText('dashOpFoot', `점프업·도소매 조정 후 기준 수익성입니다.`);

  setText('dashMargin', fmtPct(margin));
  setText('dashMarginSub', `진행중 ${prog}개 · 계획 ${planned}개`);
  setText('dashMarginFoot', `낮은 과제 추진도와 비용 구조를 같이 봐야 합니다.`);

  setText('dashKpi', `${avgKpi.toFixed(1)}점`);
  setText('dashKpiSub', `최고 ${bestName} ${Number(best.ts||0).toFixed(1)}점 · 최저 ${worstName} ${Number(worst.ts||0).toFixed(1)}점`);
  setText('dashKpiFoot', `본부 간 편차 ${gap.toFixed(1)}pt`);

  setHTML('dashBriefing', `
    <div class="dash-brief-item">
      <div class="dash-brief-badge">손익</div>
      <div class="dash-brief-text"><b>${fB(op)}억</b> 영업이익, 이익률 <b>${fmtPct(margin)}</b>입니다. 매출 규모보다 수익성 방어가 더 중요해 보이는 구간입니다.</div>
    </div>
    <div class="dash-brief-item">
      <div class="dash-brief-badge">KPI</div>
      <div class="dash-brief-text">평균 <b>${avgKpi.toFixed(1)}점</b>, 최고 <b>${bestName}</b>, 최저 <b>${worstName}</b>입니다. 잘하는 본부 복제와 낮은 본부 보정이 같이 필요합니다.</div>
    </div>
    <div class="dash-brief-item">
      <div class="dash-brief-badge">과제</div>
      <div class="dash-brief-text">전체 <b>${T.length}개</b> 중 완료 <b>${done}개</b>, 핵심 미완료 <b>${corePending.length}개</b>입니다. 회의 안건은 많은데 우선순위가 흐려질 수 있는 상태입니다.</div>
    </div>
    <div class="dash-brief-item">
      <div class="dash-brief-badge">수수료</div>
      <div class="dash-brief-text">1월 관리수수료 <b>${mgmt ? mgmt.toFixed(1)+'억' : '-'}</b>${mgmtYoY!==null ? ` · 전년동월 대비 <b>${mgmtYoY.toFixed(1)}%</b>` : ''}입니다. 흐름 점검은 수수료 탭에서 바로 이어서 보면 됩니다.</div>
    </div>`);

  setHTML('dashFocusGrid', `
    <div class="dash-focus-card good">
      <div class="dash-focus-top"><span>🏆 KPI 최고</span><b>${bestName}</b></div>
      <div class="dash-focus-main">${Number(best.ts||0).toFixed(1)}점</div>
      <div class="dash-focus-sub">우수 운영모델 벤치마킹 대상</div>
    </div>
    <div class="dash-focus-card warn">
      <div class="dash-focus-top"><span>⚠️ KPI 최저</span><b>${worstName}</b></div>
      <div class="dash-focus-main">${Number(worst.ts||0).toFixed(1)}점</div>
      <div class="dash-focus-sub">편차 원인분석 우선 대상</div>
    </div>
    <div class="dash-focus-card">
      <div class="dash-focus-top"><span>📌 핵심 과제</span><b>${corePending.length}개</b></div>
      <div class="dash-focus-main">${topTask ? topTask.nm : '이슈 없음'}</div>
      <div class="dash-focus-sub">${topTask ? `${topTask.tm} · 추진도 ${pct(topTask.pp||0)}` : '핵심 미완료 과제 없음'}</div>
    </div>
    <div class="dash-focus-card">
      <div class="dash-focus-top"><span>✅ 전체 완료율</span><b>${fmtPct(taskRate)}</b></div>
      <div class="dash-focus-main">${done}/${T.length}</div>
      <div class="dash-focus-sub">전체 과제 완료 현황</div>
    </div>`);

  const issues = [
    {tag:'HIGH', title:'핵심과제 미완료 관리', desc:`핵심과제 ${corePending.length}개가 아직 진행 중입니다. 추진도 낮은 과제부터 팀장 회의 안건으로 바로 올리는 편이 낫습니다.`},
    {tag:'WATCH', title:'KPI 편차 확대', desc:`${bestName}와 ${worstName} 간 격차가 ${gap.toFixed(1)}pt입니다. 본부별 액션플랜과 운영모델 복제가 필요합니다.`},
    {tag:'CHECK', title:'수익성 방어', desc:`영업이익률 ${fmtPct(margin)} 수준입니다. 비용 구조와 채널 믹스를 같이 점검해야 억지 성장으로 안 갑니다.`}
  ];
  if(mgmtYoY!==null){
    issues.push({tag:mgmtYoY<0?'WARN':'INFO', title:'관리수수료 흐름', desc:`1월 관리수수료가 전년동월 대비 ${mgmtYoY.toFixed(1)}% ${mgmtYoY<0?'하락':'증가'}했습니다.`});
  }
  setHTML('dashIssues', issues.map(i=>`<div class="dash-issue-item"><span class="dash-issue-tag ${i.tag.toLowerCase()}">${i.tag}</span><div><div class="dash-issue-title">${i.title}</div><div class="dash-issue-desc">${i.desc}</div></div></div>`).join(''));

  setHTML('dashQuickNote', `
    <div class="dash-quick-stat"><span>핵심</span><b>${corePending.length}개</b></div>
    <div class="dash-quick-stat"><span>완료율</span><b>${fmtPct(taskRate)}</b></div>
    <div class="dash-quick-stat"><span>최저본부</span><b>${worstName}</b></div>
  `);
}
...
normalizeProfitData();
initDashboard();loadSubscriberData();initTaskUI();initProfitUI();initKpiUI();initSubscriberUI();
