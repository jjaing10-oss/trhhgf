...
// ── 본부별 손익 파싱 (요약 시트 기준, 백만원 단위) ────────────────
function parseHqProfitExcel(wb, filename){
  try{
    const month = extractMonthFromFilename(filename);

    // 시트: '요약' 우선, 없으면 첫 번째 시트
    const wsName = wb.SheetNames.find(s=>s==='요약') || wb.SheetNames[0];
    const ws = wb.Sheets[wsName];
    if(!ws){ showUpStatus('hqprofit','err','❌ 요약 시트를 찾을 수 없습니다.'); return; }
    const rows = XLSX.utils.sheet_to_json(ws,{header:1, defval:null});

    // ── 열 탐지: '강북본부','강남본부'... 포함된 헤더 행 찾기 ──
    const hqOrder = ['강북본부','강남본부','강서본부','동부본부','서부본부'];
    let colMap = {};
    let headerRowIdx = -1;
    for(let ri=0; ri<Math.min(rows.length,10); ri++){
      const r = rows[ri];
      const found = hqOrder.filter(nm=>r.some(c=>String(c||'').includes(nm.replace('본부',''))));
      if(found.length>=4){ headerRowIdx=ri; break; }
    }
    if(headerRowIdx>=0){
      const hr = rows[headerRowIdx];
      hr.forEach((cell,ci)=>{
        const s = String(cell||'');
        hqOrder.forEach(nm=>{ if(s.includes(nm.replace('본부',''))) colMap[nm]=ci; });
      });
    }
    // fallback: 강북=F(5),강남=G(6),강서=H(7),동부=I(8),서부=J(9)
    if(Object.keys(colMap).length<5){
      colMap={강북본부:5,강남본부:6,강서본부:7,동부본부:8,서부본부:9};
    }

    // ── 행 탐지: 계정명 공백제거 후 키워드 매칭 ──
    let revRow=-1, gpRow=-1, sgaRow=-1;
    rows.forEach((r,ri)=>{
      const nm = String(r[2]||r[0]||'').replace(/\s/g,'');
      if(nm==='매출'&&revRow<0) revRow=ri;
      else if(nm==='매출총이익'&&gpRow<0) gpRow=ri;
      else if((nm==='판매비와관리비'||nm==='판매비와일반관리비'||nm==='판관비')&&sgaRow<0) sgaRow=ri;
    });
    // fallback
    if(revRow<0) revRow=4;
    if(gpRow<0)  gpRow=11;
    if(sgaRow<0) sgaRow=12;

    // ── 단위 감지: row[0][9]==1000000 이면 백만원, 아니면 원화 ──
    const unitCell = Number(rows[0]?.[9]||0);
    const multiplier = (unitCell>=100000) ? unitCell : 1;

    // ── D.hq 갱신 ──
    const newHq = D.hq.map(h=>{
      const ci = colMap[h.nm];
      if(ci===undefined) return h;
      const rev = Math.round((Number(rows[revRow]?.[ci])||0) * multiplier);
      const gp  = Math.round((Number(rows[gpRow]?.[ci])||0) * multiplier);
      const sga = Math.round((Number(rows[sgaRow]?.[ci])||0) * multiplier);
      return {...h, rev, gp, sga};
    });
    D.hq = newHq;
    ...
    const lines = newHq.map(h=>{
      const op = h.gp - h.sga;
      return `  ${h.nm}: 매출 ${(h.rev/1e8).toFixed(0)}억 / 영업이익 ${(op/1e8).toFixed(1)}억 (${(op/h.rev*100).toFixed(1)}%)`;
    });
