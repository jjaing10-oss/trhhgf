...
function setTabMonth(type, month){
  ...
  const tabMap={tasks:'tasks',profit:'profit',kpi:'kpi',subscriber:'subscriber',commission:'commission'};
  if(tabMap[type]===currentTab) syncPeriodForTab(currentTab);
  updateTabMonthBadges();
}
...
function formatMonthLabel(month){
  ...
  return m ? `${m[1]}.${m[2]}월` : month;
}
function getTabMonth(type){
  const m=getSavedTabMonths();
  return m[type]||null;
}
function getPeriodLabelForTab(tab){
  const keyMap={tasks:'tasks',profit:'profit',kpi:'kpi',subscriber:'subscriber',commission:'commission'};
  const key=keyMap[tab];
  const month=key?getTabMonth(key):null;
  const fallback=(D.baseMonth || (typeof D.period==='string' && D.period.match(/\d{4}\.\d{2}/) ? D.period.match(/\d{4}\.\d{2}/)[0] : null));
  const pick=month||fallback;
  return pick ? (pick+' 기준') : (D.period||'-');
}
function syncPeriodForTab(tab){
  const p=getPeriodLabelForTab(tab);
  if(p) D.period=p;
}
...
function updateTabMonthBadges(){
  ...
  mapping.forEach(([id,val])=>{
    ...
    if(el) el.textContent='기준월 '+formatMonthLabel(val||'-');
  });
}
