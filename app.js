...
function rptStyle(){
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  body{font-family:'Noto Sans KR',sans-serif;background:#f1f5f9;color:#0f172a;padding:24px 0 48px;font-size:12px;line-height:1.65}
  .page{max-width:860px;margin:0 auto;padding:32px 36px 48px;background:#fff;border-radius:16px;box-shadow:0 4px 24px rgba(15,23,42,.08),0 1px 4px rgba(15,23,42,.05)}
  .cover{background:linear-gradient(135deg,#0369a1 0%,#0c4a6e 50%,#082f49 100%);color:#fff;padding:40px 28px 32px;margin-bottom:0}
  .cover-tag{display:inline-block;padding:4px 12px;background:rgba(255,255,255,.15);border:1px solid rgba(255,255,255,.3);border-radius:20px;font-size:10px;font-weight:700;letter-spacing:1px;text-transform:uppercase;margin-bottom:16px}
  .cover-title{font-size:24px;font-weight:900;letter-spacing:-.5px;margin-bottom:8px;line-height:1.2}
  .cover-sub{font-size:13px;color:rgba(255,255,255,.7);margin-bottom:24px}
  .cover-meta{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;border-top:1px solid rgba(255,255,255,.2);padding-top:20px}
  .cover-kpi{text-align:center}
  .cover-kpi-v{font-size:28px;font-weight:900;letter-spacing:-1px}
  .cover-kpi-l{font-size:10px;color:rgba(255,255,255,.6);font-weight:500;margin-top:2px}
  ...
  @media print{.page{padding:20px}h1{font-size:15px}.cover{padding:24px 20px 20px}}
  </style>`;
}
...
function parseTaskExcel(wb, filename){
  ...
  const ws=wb.Sheets['data'];if(!ws){showUpStatus('task','err','❌ "data" 시트 없음');return;}const rows=XLSX.utils.sheet_to_json(ws,{header:1,defval:''});const tasks=[];for(let i=5;i<rows.length;i++){const r=rows[i];if(!r[5]||!r[1])continue;tasks.push({no:r[1],ch:r[2]||'',tm:r[3]||'',co:r[4]==='●'?'●':'',nm:String(r[5]).replace(/\n/g,' ').trim(),mg:String(r[6]||'').trim(),sc:String(r[7]||'').trim(),plan:String(r[8]||'').trim(),pg:String(r[9]||'').trim(),st:r[10]||'계획',cp:r[11]||'미완료',pp:parseFloat(r[12])||0});}
  if(tasks.length>0){D.tasks=tasks;initTaskUI();showUpStatus('task','ok','✅ '+tasks.length+'개 과제 로드!'+(month?` · 기준월 ${month}`:''));}
...
function parseKpiExcel(wb, filename){try{
  ...
  if(kpi.length>=3){kpi.sort((a,b)=>b.ts-a.ts);kpi.forEach((r,i)=>r.rk=i+1);D.kpi=kpi;initKpiUI();showUpStatus('kpi','ok','✅ KPI 업데이트!'+(month?` · 기준월 ${month}`:''));}
...
normalizeProfitData();
loadSubscriberData();initTaskUI();initProfitUI();initKpiUI();initSubscriberUI();
...
// ===== Executive Briefing Upgrade vNext =====

function getExecutiveChannelRanks(){
  const P = D.profit || {};
  const rows = CHS.map(key=>{
    const rev = Number((P.revenue||{})[key]||0);
    const op  = Number((P.op||{})[key]||0);
    const opm = rev>0 ? (op/rev*100) : 0;
    return {key, name:CN[key]||key, rev, op, opm};
  }).sort((a,b)=>b.opm-a.opm);
  return rows;
}

function getAutoExecutiveBrief(){
  const E = buildBriefingEngine();
  const ranks = getExecutiveChannelRanks();
  const best = ranks[0] || {name:'-', opm:0};
  const worst = ranks[ranks.length-1] || {name:'-', opm:0};
  const kTop = E.K && E.K[0] ? E.K[0] : null;
  const kBot = E.K && E.K[E.K.length-1] ? E.K[E.K.length-1] : null;
  const items = [
    {tag:'손익', text:`전사 영업이익은 ${fB(Number(E.P?.op?.total||0))}억, 이익률은 ${pctStr(ratio(E.P?.op?.total,E.P?.revenue?.total))}입니다. 채널 수익성 1위는 ${best.name}(${best.opm.toFixed(1)}%), 최하위는 ${worst.name}(${worst.opm.toFixed(1)}%)입니다.`},
    {tag:'KPI', text:kTop&&kBot?`KPI 1위는 ${kTop.hq}(${kTop.ts.toFixed(1)}점), 최하위는 ${kBot.hq}(${kBot.ts.toFixed(1)}점)이며 격차는 ${E.kpiGap.toFixed(1)}p입니다.`:'KPI 데이터가 아직 충분하지 않습니다.'},
    {tag:'가입자', text:`무선 전체 ${fmtSubNum(Number(E.sub?.wirelessTot||0))}명, 관리수수료 대상은 ${fmtSubNum(Number(E.sub?.mgmtCur||0))}명입니다. 관리수수료 대상은 연초 대비 ${E.sub?.mgmtChg||'-'}% 변동했습니다.`},
    {tag:'과제', text:`전사 과제 완료율은 ${E.T.length?((E.done/E.T.length)*100).toFixed(1):'0.0'}%이며, 미착수 ${E.zeroProgress.length}건과 핵심 잔여 ${E.coreOpen.length}건을 주간 모니터링해야 합니다.`}
  ];
  return {E, ranks, items};
}

async function exportReportPdf(){
  const body = document.getElementById('rptViewerBody');
  const iframe = document.getElementById('rptIframe');
  const title = (window._currentReportTitle || '보고서').replace(/[\\/:*?"<>|]/g,'_');
  if(!body || !iframe){
    alert('먼저 보고서를 열어주세요.');
    return;
  }
  try{
    ...
    const target = targetDoc && targetDoc.body ? targetDoc.body : null;
...
// override initBriefing with cleaner executive layout
function initBriefing(){
  const X = getAutoExecutiveBrief();
  const E = X.E;
  const s = E.sub || {};
  const kTop = E.K && E.K[0] ? E.K[0] : null;
  const kBot = E.K && E.K[E.K.length-1] ? E.K[E.K.length-1] : null;
  const avgKpi = E.K && E.K.length ? (E.K.reduce((a,b)=>a+(Number(b.ts)||0),0)/E.K.length) : 0;
  const ranks = X.ranks;
  const maxOpm = Math.max(...ranks.map(r=>Math.abs(r.opm)),1);
  const hmRows = (E.K||[]).map(k=>{
    const weak = [{n:'소매',v:k.rt.t},{n:'도매',v:k.wh.t},{n:'소상공',v:k.sm.t}].sort((a,b)=>a.v-b.v)[0];
    const pill = v => v>=80 ? 'good' : v>=65 ? 'mid' : 'high';
    return `<tr>
      <td style="font-weight:800">${k.hq.replace('본부','')}</td>
      <td><span class="ex-pill ${pill(k.rt.t)}">${k.rt.t.toFixed(1)}</span></td>
      <td><span class="ex-pill ${pill(k.wh.t)}">${k.wh.t.toFixed(1)}</span></td>
      <td><span class="ex-pill ${pill(k.sm.t)}">${k.sm.t.toFixed(1)}</span></td>
      <td style="color:#fca5a5;font-weight:700">${weak.n}</td>
    </tr>`;
  }).join('');
  const highActions = (E.actions||[]).filter(a=>a.priority==='HIGH').slice(0,4);
  const html = `
  <div class="ex-brief">
    <div class="ex-top">
      <div class="ex-cover">
        <div class="ex-eyebrow">Executive Dashboard</div>
        <div class="ex-title">KT M&S 경영 브리핑</div>
        <div class="ex-mini">설명문은 버리고 숫자와 편차만 남겼습니다. 대시보드는 원래 그래야 하니까.</div>
        <div class="ex-submeta">
          <span class="ex-chip">📅 ${D.period}</span>
          <span class="ex-chip">⚠️ HIGH ${highActions.length}건</span>
          <span class="ex-chip">📡 가입자 · 손익 · KPI 통합</span>
        </div>
      </div>
      <div class="ex-focus">
        <div class="ex-focus-kicker">Focus</div>
        <div class="ex-focus-row">
          <div>
            <div class="ex-focus-lbl">핵심 리스크</div>
            <div class="ex-mini">관리수수료 / H·C / 하위 KPI</div>
          </div>
          <div>
            <div class="ex-focus-val">${highActions.length}건</div>
            <div class="ex-focus-sub">즉시 점검 필요</div>
          </div>
        </div>
        <div class="ex-focus-row">
          <div>
            <div class="ex-focus-lbl">KPI 1위</div>
            <div class="ex-mini">${kTop?kTop.hq:'-'}</div>
          </div>
          <div>
            <div class="ex-focus-val">${kTop?kTop.ts.toFixed(1):'-'}</div>
            <div class="ex-focus-sub">최하위 ${kBot?kBot.hq:'-'}</div>
          </div>
        </div>
        <div class="ex-focus-row">
          <div>
            <div class="ex-focus-lbl">채널 수익성 1위</div>
            <div class="ex-mini">${ranks[0]?ranks[0].name:'-'}</div>
          </div>
          <div>
            <div class="ex-focus-val">${ranks[0]?ranks[0].opm.toFixed(1):'-'}%</div>
            <div class="ex-focus-sub">최저 ${ranks[ranks.length-1]?ranks[ranks.length-1].name:'-'}</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="ex-kpi-grid">
      <div class="ex-kpi">
        <div class="ex-kpi-lbl">전사 매출</div>
        <div class="ex-kpi-val">${fB(Number(E.P?.revenue?.total||0))}억</div>
        <div class="ex-kpi-sub">월 기준 총매출. 채널 믹스와 상품/서비스 비중 확인 필요</div>
      </div>
      <div class="ex-kpi">
        <div class="ex-kpi-lbl">영업이익</div>
        <div class="ex-kpi-val">${fB(Number(E.P?.op?.total||0))}억</div>
        <div class="ex-kpi-sub">영업이익률 ${pctStr(ratio(E.P?.op?.total,E.P?.revenue?.total))} · 하위 채널 ${ranks[ranks.length-1]?ranks[ranks.length-1].name:'-'}</div>
      </div>
      <div class="ex-kpi">
        <div class="ex-kpi-lbl">평균 KPI</div>
        <div class="ex-kpi-val">${avgKpi.toFixed(1)}</div>
        <div class="ex-kpi-sub">본부간 격차 ${E.kpiGap.toFixed(1)}p · 소상공 편차 ${E.smbGap.toFixed(1)}p</div>
      </div>
      <div class="ex-kpi">
        <div class="ex-kpi-lbl">관리수수료 대상</div>
        <div class="ex-kpi-val">${fmtSubNum(Number(s.mgmtCur||0))}</div>
        <div class="ex-kpi-sub">연초 대비 ${s.mgmtChg||'-'}% · 구조적 감소 여부 주시</div>
      </div>
    </div>
    ...
    <div class="ex-grid">
      <div class="ex-card">
        <h3>🤖 AI 자동 경영 브리핑</h3>
        <div class="ex-brief-list">
          ${X.items.map(x=>`<div class="ex-brief-item"><strong>${x.tag}</strong><span>${x.text}</span></div>`).join('')}
        </div>
      </div>
      <div class="ex-card">
        <h3>📊 채널 순위 그래프</h3>
        <div class="ex-rank-list">
          ${ranks.map((r,i)=>`<div class="ex-rank-row">
            <div class="ex-rank-name">${i+1}. ${r.name}</div>
            <div class="ex-rank-bar"><div class="ex-rank-fill" style="width:${Math.max(8,Math.abs(r.opm)/maxOpm*100)}%"></div></div>
            <div class="ex-rank-val">${r.opm.toFixed(1)}%</div>
          </div>`).join('')}
        </div>
        <div class="ex-mini" style="margin-top:10px">기준: 채널별 영업이익률(OPM) 순위</div>
      </div>
    </div>
    ...
    <div class="ex-grid">
      <div class="ex-card">
        <h3>🎯 KPI Heatmap</h3>
        <table class="ex-heat">
          <thead><tr><th>본부</th><th>소매</th><th>도매</th><th>소상공</th><th>약점</th></tr></thead>
          <tbody>${hmRows}</tbody>
        </table>
      </div>
      <div class="ex-card">
        <h3>⚠️ 액션 포인트</h3>
        <div class="ex-action-list">
          ${highActions.map(a=>`<div class="ex-action"><div><span class="ex-pill high">HIGH</span></div><div><h4>${a.item}</h4><p>${a.detail}</p></div></div>`).join('')}
        </div>
      </div>
    </div>
    ...
    <div class="ex-card">
      <h3>🔗 바로가기</h3>
      <div class="ex-shortcuts">
        <button class="ex-btn" onclick="switchTab('profit')">BM 손익</button>
        <button class="ex-btn" onclick="switchTab('kpi')">KPI 분석</button>
        <button class="ex-btn" onclick="switchTab('subscriber')">가입자 분석</button>
        <button class="ex-btn" onclick="switchTab('commission')">수수료 분석</button>
        <button class="ex-btn dark" onclick="genMonthlyBriefing()">경영 보고서</button>
        <button class="ex-btn dark" onclick="genCeoBriefing()">CEO 요약</button>
      </div>
    </div>
  </div>`;
  const el = document.getElementById('briefingSnapshot');
