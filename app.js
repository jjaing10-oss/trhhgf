...
function initDashboard(){
  try{
    ...
    const SD = (typeof subscriberData!=='undefined' && subscriberData) ? subscriberData
             : (typeof DEFAULT_SUBSCRIBER_DATA!=='undefined' ? DEFAULT_SUBSCRIBER_DATA : null);
    if(SD && SD.wireless){
      const w = SD.wireless;
      if(el('dashWireless')) el('dashWireless').textContent = (w.total/10000).toFixed(1)+'만';
      if(el('dashWirelessSub')){
        const na = w.netAdd||0;
        const sign = na>=0?'+':'';
        el('dashWirelessSub').textContent = '순증 '+sign+na.toLocaleString()+' · 해지 '+(w.churn||0).toLocaleString();
      }
      if(el('dashCapa')) el('dashCapa').textContent = (w.capa||0).toLocaleString();
      if(el('dashCapaSub')){
        const series = w.series||{};
        const newS = series.newSubs ? series.newSubs[series.newSubs.length-1] : 0;
        const chgS = series.chgSubs ? series.chgSubs[series.chgSubs.length-1] : 0;
        el('dashCapaSub').textContent = '신규 '+newS.toLocaleString()+' · 기변 '+chgS.toLocaleString();
      }
    }
    // 서비스 매출
    if(el('dashSvcRev') && P.rev_service){
      el('dashSvcRev').textContent = fmt(P.rev_service.total);
      if(el('dashSvcRevSub')){
        const prd = P.rev_product ? P.rev_product.total : 0;
        const svcRatio = P.rev_service.total && revenue ? (P.rev_service.total/revenue*100).toFixed(1) : '0';
    ...
    if(el('dashMgmtFee')){
      ...
      if(mf && mf.total){
        ...
        if(el('dashMgmtFeeSub')){
          el('dashMgmtFeeSub').textContent = '소매 '+(mf.retail||0)+'억 · 도매 '+(mf.wholesale||0)+'억';
        }
      } else if(comm.monthly25 && comm.monthly25[0]){
        const jan = comm.monthly25[0];
        el('dashMgmtFee').textContent = jan.total.toFixed(1)+'억';
        if(el('dashMgmtFeeSub')){
          el('dashMgmtFeeSub').textContent = '소매 '+jan.retail.toFixed(1)+'억 · 도매 '+jan.wholesale.toFixed(1)+'억';
        }
      }
    }
    ...
    if(chGrid && P.revenue){
      ...
      chGrid.innerHTML = channels.map(c=>{
