...
function parseHqProfitExcel(wb, filename){
  try{
    // 파일명에서 연/월 감지 (예: 26년_1월_본부별_손익)
    const month = detectMonth(filename);

    // Sheet1 (첫 번째 시트)
    const wsName = wb.SheetNames[0];
    const ws = wb.Sheets[wsName];
    if(!ws){ showUpStatus('hqprofit','err','❌ 시트 없음'); return; }
    const rows = XLSX.utils.sheet_to_json(ws,{header:1, defval:0});

    // 열 헤더 자동 탐지 (행2에서 본부명 검색)
    // 기본 구조: col0=계정명, col1=합계, col2=경영기획총괄, col3=영업총괄, col4=감사실
    //            col5=강북, col6=강남, col7=강서, col8=동부, col9=서부, col10=본사공통비용
    const headerRow = rows[1] || [];
    const colMap = {};
    const hqNameMap = {
      '강북':'강북본부','강남':'강남본부','강서':'강서본부',
      '동부':'동부본부','서부':'서부본부'
    };
    headerRow.forEach((cell,ci)=>{
      const s = String(cell||'');
      for(const [key,nm] of Object.entries(hqNameMap)){
        if(s.includes(key)) colMap[nm] = ci;
      }
    });
    // fallback: 헤더 탐지 실패 시 고정 열 사용
    if(Object.keys(colMap).length < 5){
      colMap['강북본부']=5; colMap['강남본부']=6; colMap['강서본부']=7;
      colMap['동부본부']=8; colMap['서부본부']=9;
    }

    // 계정명으로 행 탐지 (공백 제거 후 비교)
    let revRow=-1, gpRow=-1, sgaRow=-1;
    rows.forEach((r,ri)=>{
      const nm = String(r[0]||'').replace(/\s/g,'');
      if(nm==='매출'||nm==='매출액') revRow=ri;
      else if(nm==='매출총이익') gpRow=ri;
      else if(nm==='판매비와일반관리비'||nm==='판관비') sgaRow=ri;
    });
    // fallback: 고정 행 인덱스
    if(revRow<0) revRow=3;
    if(gpRow<0) gpRow=8;
    if(sgaRow<0) sgaRow=9;

    const newHq = D.hq.map(h=>{
      const ci = colMap[h.nm];
      if(ci===undefined) return h; // 본부 매핑 없으면 기존값 유지
      const rev = Number(rows[revRow]?.[ci])||0;
      const gp  = Number(rows[gpRow]?.[ci])||0;
      const sga = Number(rows[sgaRow]?.[ci])||0;
      return {...h, rev, gp, sga};
    });
    ...
    initProfitUI();
    const msg = `✅ 본부별 손익 업데이트!${month?` · 기준월 ${month}`:''}\n` +
      newHq.map(h=>`  ${h.nm}: 매출 ${(h.rev/1e8).toFixed(1)}억 / 영업이익 ${((h.gp-h.sga)/1e8).toFixed(1)}억`).join('\n');
    showUpStatus('hqprofit','ok', msg);
  }catch(err){ showUpStatus('hqprofit','err','❌ '+err.message); }
}
