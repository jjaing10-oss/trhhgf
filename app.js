...
function parseHqProfitExcel(wb, filename){
  try{
    ...
    // ── 열 탐지: 행2(index 1)에서 본부명 포함 여부로 열 인덱스 매핑 ──
    const headerRow = rows[1] || [];
    const hqKeyMap = {'강북':'강북본부','강남':'강남본부','강서':'강서본부','동부':'동부본부','서부':'서부본부'};
    const colMap = {};
    headerRow.forEach((cell,ci)=>{
      const s = String(cell||'');
      for(const [key,nm] of Object.entries(hqKeyMap)){
        if(s.includes(key)) colMap[nm] = ci;
      }
    });
    // fallback: 탐지 실패 시 고정 열 (강북=5, 강남=6, 강서=7, 동부=8, 서부=9)
    if(Object.keys(colMap).length < 5){
      Object.assign(colMap,{강북본부:5,강남본부:6,강서본부:7,동부본부:8,서부본부:9});
    }

    // ── 행 탐지: 계정명 공백 제거 후 키워드 매칭 ──
    let revRow=-1, gpRow=-1, sgaRow=-1;
    rows.forEach((r,ri)=>{
      const nm = String(r[0]||'').replace(/\s/g,'');
      if((nm==='매출'||nm==='매출액') && revRow<0) revRow=ri;
      else if(nm==='매출총이익' && gpRow<0) gpRow=ri;
      else if((nm==='판매비와일반관리비'||nm==='판관비') && sgaRow<0) sgaRow=ri;
    });
    ...
    const newHq = D.hq.map(h=>{
      ...
      return {
        ...h,
        rev: Number(rows[revRow]?.[ci])||0,
        gp:  Number(rows[gpRow]?.[ci])||0,
        sga: Number(rows[sgaRow]?.[ci])||0
      };
    });
    D.hq = newHq;
    ...
    const lines = newHq.map(h=>{
      const op = h.gp - h.sga;
      return `  ${h.nm}: 매출 ${(h.rev/1e8).toFixed(0)}억 / 영업이익 ${(op/1e8).toFixed(0)}억 (${(op/h.rev*100).toFixed(1)}%)`;
    });
    showUpStatus('hqprofit','ok',`✅ 본부별 손익 업데이트!${month?` · 기준월 ${month}`:''}\n${lines.join('\n')}`);
